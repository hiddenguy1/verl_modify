...
class RayPPOTrainer:
    ...
    def _validate_config(self):
        ...
        ## 新增
        # Add GROUP algorithm specific validations
        if config.algorithm.adv_estimator == AdvantageEstimator.GROUP:
            group_params = config.algorithm.get("group_params", {})
            
            # 验证必需参数
            if group_params.get("group_variance_threshold", 1.0) <= 0:
                raise ValueError("group_variance_threshold must be positive")
            if group_params.get("group_max_size", 32) < 2:
                raise ValueError("group_max_size must be at least 2")
            
            # 确保critic可用
            if not self.use_critic:
                raise ValueError("GROUP algorithm requires critic model")
            
            # 建议rollout.n > 1
            if config.actor_rollout_ref.rollout.n < 2:
                print("Warning: GROUP algorithm works best with rollout.n > 1")
        ## end新增
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.GROUP:
                            batch_result = compute_advantage(
                                batch,
                            ...
                            if isinstance(batch_result, tuple) and len(batch_result) == 3:
                                batch, group_metrics, monitoring_metrics = batch_result
                                metrics.update(group_metrics)
                                metrics.update(monitoring_metrics)
                                
                                # 🆕 每N步打印一次详细信息
                                if self.global_steps % 10 == 0:  # 每10步打印一次
