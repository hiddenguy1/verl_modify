...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        self._init_group_support()
    
    ## 新增
    def _init_group_support(self):
        """
        🆕 新增：初始化GROUP算法支持
        完全独立的初始化，不影响现有功能
        """
        ...
        if algorithm_config:
            self.is_group_algorithm = getattr(algorithm_config, 'adv_estimator', None) == 'group_ppo'
            self.group_params = getattr(algorithm_config, 'group_params', {})
        else:
            self.is_group_algorithm = False
            self.group_params = {}
        ...
        # 稀疏检测阈值
        self.sparse_threshold = self.group_params.get('sparse_threshold', 0.1)
        
        if self.is_group_algorithm and hasattr(self, 'rank') and self.rank == 0:
            print(f"🎯 GROUP algorithm support initialized with params: {self.group_params}")
    ...
    def update_actor(self, data: DataProto):
        """
        最小侵入性增强的 update_actor 方法
        添加 GROUP 算法支持，同时保持所有现有功能完全不变
        """
        # 🔥 保持所有现有的前置处理完全不变
        data = data.to(get_torch_device().current_device())
        ...
        try:
            ...
            if hasattr(self, 'rank') and self.rank == 0:
                print(f"🚨 Actor update failed ({e}), falling back to standard path")
            output = self._update_actor_standard_path(data)
    ...
    def _update_actor_standard_path(self, data: DataProto) -> DataProto:
        """
        🔥 原有的标准更新路径，保持100%不变
        这就是原来的 update_actor 的核心逻辑
        """
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
    ...
    def _update_actor_with_group_optimization(self, data: DataProto) -> DataProto:
        """
        🆕 新增：GROUP 算法的优化更新路径
        """
        if hasattr(self, 'rank') and self.rank == 0:
            print("🎯 Using GROUP algorithm optimization")
        
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            
            # GROUP 算法特定的处理
            with Timer(name="update_policy_group", logger=None) as timer:
                metrics = self._update_policy_with_group_optimization(data)
    ...
    def _update_policy_with_group_optimization(self, data: DataProto) -> dict:
        """
        🆕 新增：GROUP 算法的核心策略更新逻辑
        """
        advantages = data.batch["advantages"]
        response_mask = data.batch["response_mask"]
        ...
        self.group_stats['total_updates'] += 1
        self.group_stats['group_updates'] += 1
        self.group_stats['compression_ratios'].append(compression_ratio)
        
        if active_positions > 0:
            # 🎯 核心：为 GROUP 算法在 meta_info 中添加优化标记
            # 供下游的 actor.update_policy 使用
            data.meta_info["group_optimization"] = True
            data.meta_info["endpoint_mask"] = non_zero_adv_mask
            data.meta_info["compression_ratio"] = compression_ratio
            data.meta_info["active_positions"] = active_positions
            data.meta_info["total_positions"] = total_positions
            
            # 调用标准的 update_policy，但传入 GROUP 标记信息
            metrics = self.actor.update_policy(data=data)
            
            # 计算效率增益
            efficiency_gain = 1.0 / max(1.0 - compression_ratio, 0.01)
            self.group_stats['efficiency_gains'].append(efficiency_gain)
            
            # 添加 GROUP 特定的指标
            metrics.update({
                "group/compression_ratio": compression_ratio,
                "group/active_positions": active_positions,
                "group/total_positions": total_positions,
                "group/efficiency_gain": efficiency_gain,
                "group/memory_saved_ratio": compression_ratio,
                # 滑动窗口统计（最近100次）
                "group/avg_compression": np.mean(self.group_stats['compression_ratios'][-100:]),
                "group/avg_efficiency": np.mean(self.group_stats['efficiency_gains'][-100:]),
                "group/total_group_updates": self.group_stats['group_updates'],
            })
            
            if hasattr(self, 'rank') and self.rank == 0:
                print(f"✅ GROUP optimization: {active_positions}/{total_positions} active positions "
                    f"(compression: {compression_ratio:.1%}, speedup: {efficiency_gain:.1f}x)")
            
        else:
            # 没有 active 位置，跳过更新
            if hasattr(self, 'rank') and self.rank == 0:
                print("⚠️ GROUP: No active positions found, skipping policy update")
            ...
            metrics = {
                ...
                "actor/ppo_kl": 0.0,
                "group/compression_ratio": 1.0,
                "group/active_positions": 0,
                "group/total_positions": total_positions,
                "group/efficiency_gain": float('inf'),
                "group/no_update_skipped": 1.0,  # 标记跳过了更新
            }
    ...
    def get_group_statistics(self) -> dict:
        ...
        if stats['efficiency_gains']:
            stats['avg_efficiency_gain'] = np.mean(stats['efficiency_gains'])
            stats['max_efficiency_gain'] = np.max(stats['efficiency_gains'])
            
        if stats['total_updates'] > 0:
            stats['group_usage_ratio'] = stats['group_updates'] / stats['total_updates']
            
        return stats

    def reset_group_statistics(self):
        """
        🆕 新增：重置 GROUP 算法统计
        """
        self.group_stats = {
